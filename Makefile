...
push-images: ## Build + push das imagens Docker. Uso: make push-images TAG=v0.1.0
ifndef TAG
	$(error Defina a tag: make push-images TAG=v0.1.0)
endif
	@# Build no próprio host (runner do CI ou sua máquina) e push direto ao
	@# Artifact Registry. Sem Cloud Build: o runner do GitHub já tem Docker,
	@# e isso evita depender da API cloudbuild (e da sua propagação no 1º deploy).
	gcloud auth configure-docker $(GCP_REGION)-docker.pkg.dev --quiet
	docker build -t $(REGISTRY)/gateway:$(TAG) gateway
	docker push $(REGISTRY)/gateway:$(TAG)
	docker build -t $(REGISTRY)/broker:$(TAG) broker
